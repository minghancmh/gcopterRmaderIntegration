...
SolverNlopt::SolverNlopt(ms::par_solver &par)
{
  ...
  std::cout<<"==============Initializing a SolverNLopt Object==================="<<std::endl;
...
void SolverNlopt::generateAStarGuess(bool& is_stuck, bool& is_A_star_failed, bool& is_q0_fail)
{
  std::cout<<"==============Generating A Star Guess==================="<<std::endl;
  ...
  double goal_size = 0.05;  //[meters] // original 0.05
...
{
  std::cout<<"All Constraints"<<red<<constraints[5]<<std::endl;
                                             ...
                                             std::vector<double> &d)
{
  ...
  std::cout << "Infeasible Constraints Highlighted in Red\n";
  for (int i = 0; i < num_of_constraints_; i++)
  {
    if (constraints[i] > epsilon_tol_constraints_)  // constraint is not satisfied yet
    {
      std::cout << on_red << std::setprecision(5) << "Constraint " << i << " = " << constraints[i] <<reset<< std::endl;
    }
    // else{
    //   std::cout << std::setprecision(5) << "Constraint" << i << "=" << constraints[i] << std::endl;
    // }
  ...
  // std::cout << "All the constraints are as follows: \n";
...

{
  ...
  generateAStarGuess(is_stuck, is_A_star_failed, is_q0_fail);
  ...
  // std::cout<<red<<"All constraints"<< tol_constraints <<std::endl;

  

  printInfeasibleConstraints(q_guess_, n_guess_, d_guess_); //line 598
