...
class ObsMissionModePlugin(Plugin):

    def __init__(self, context):
        super(ObsMissionModePlugin, self).__init__(context)
        # Give QObjects reasonable names
        self.setObjectName('ObsMissionModePlugin')  
        ...
        # Get path to UI file which should be in the "resource" folder of this package
        # ui_file = os.path.join(rp.get_path('behavior_selector'), 'resource', 'MissionModePlugin.ui')
        ui_file = os.path.join(rp.get_path('behavior_selector'), 'resource', 'ObsMissionModePlugin.ui')
        # Extend the widget with all attributes and children from UI file
        loadUi(ui_file, self._widget)
        # Give QObjects reasonable names
        self._widget.setObjectName('ObsMissionModePluginUi')
        # Show _widget.windowTitle on left-top of each plugin (when 
        # it's set in _widget). This is useful when you open multiple 
        # plugins at once. Also if you open multiple instances of your 
        # plugin at once, these lines add number to make it easy to 
